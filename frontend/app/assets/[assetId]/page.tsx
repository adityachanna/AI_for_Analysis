...
import { api, Asset, AuditSummary, DemoClip, GraphData, ScanResult, Violation } from "@/lib/api";
    ...
    <div className="page">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'flex-end', flexShrink: 0 }}>
          <button onClick={runScan} disabled={loading} style={{ minWidth: '200px' }}>
            {loading ? "Scanning All Platforms..." : "Run Detection Pipeline"}
          </button>
              ...
              {scan && (
                <div className="timeline">
                  {scan.stages.map((stage, i) => {
                    ...
                    const stageLabel = stageNum === 1 ? 'A — dHash' : stageNum === 2 ? 'B — Embedding' : 'C — Gemini';
                    return (
                  ...
                  <div className="grid three">
                    {graph.nodes.map((node) => (
                      <div key={node.id} style={{
                        ...
                        <span className={`pill ${node.type === 'asset' ? 'good' : node.type === 'violation' ? 'bad' : 'warn'}`} style={{ alignSelf: 'flex-start', fontSize: '0.6rem' }}>
